...
export default function PaymentScheduleDetails() {
  ...
  const handlePaymentSuccess = async () => {
    if (projectId) {
      try {
        ...
        toast.error(
          "Payment successful but failed to update schedule. Please refresh the page.",
        );
      }
    }
  };
    ...
    return (
      <div className="text-center py-8">
        ...
        <p className="text-gray-600 mt-2">
          The requested payment schedule could not be found.
        </p>
      </div>
    );
  }
  ...
  const completionPercentage =
    ((currentSchedule?.totalPaid ?? 0) /
      (currentSchedule?.totalProjectValue ?? 1)) *
    100;
  const currentMilestone = currentSchedule?.milestones?.find(
  ...
  const getCurrentMilestoneCumulative = () => {
    if (!currentSchedule?.milestones || !currentSchedule?.currentMilestone)
      return { totalDue: 0, totalPaid: 0, amountDue: 0 };

    const milestonesUpToCurrent = currentSchedule.milestones.filter(
      (m) => m.slNo <= currentSchedule.currentMilestone,
    );

    const totalDue = milestonesUpToCurrent.reduce(
      (sum, m) => sum + m.amount,
      0,
    );
    const totalPaid = milestonesUpToCurrent.reduce(
      (sum, m) => sum + m.actualPaid,
      0,
    );
    ...
    <div className="sm:p-4 lg:p-6 space-y-4 sm:space-y-6">
          ...
          <div className="space-y-4 sm:space-y-6">
            <div>
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                <Badge
                  variant="secondary"
                  className="py-1.5 px-3 bg-slate-100 text-slate-700 w-fit"
                >
                  Project ID:{" "}
                  {currentSchedule?.projectId?._id?.slice(-8)?.toUpperCase()}
                </Badge>
                <Badge
                  variant="secondary"
                  className={cn(
                  ...
                  <div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
                      <span className="text-base sm:text-lg font-medium text-slate-900 break-all">
                        {formatCurrency(currentMilestone.actualPaid)} /{" "}
                        {formatCurrency(currentMilestone.amount)}
                      </span>
                      <Badge
                        variant="secondary"
                        className={cn(
                  ...
                  return (
                    <tr
                      ...
                      className={cn(
                        ...
                        isCurrent &&
                          "bg-gradient-to-r from-rose-50 to-rose-100 hover:from-rose-100 hover:to-rose-150",
                      )}
                    >
          ...
          <div className="flex flex-col sm:flex-row items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm sm:text-base text-slate-700 leading-relaxed">
                Please ensure timely payments according to the schedule above to
                avoid any delay in project completion. For any payment-related
                queries, contact our finance department.
              </p>
            </div>
          </div>
        </CardContent>
      ...
      {currentSchedule && (
        <PaymentModal
          ...
          totalRemaining={
            cumulativePayment.amountDue > 0
              ? cumulativePayment.amountDue
