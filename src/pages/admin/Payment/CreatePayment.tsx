        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-2 gap-6">
                    ...
                    <PopoverContent className="w-[400px] p-0">
                      <Command>
                        ...
                        <CommandGroup heading="Projects">
                          {Array.isArray(searchResults) && searchResults.map((project) => (
                            <CommandItem
                              key={project._id}
                              value={project.title}
                              onSelect={async () => {
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium">Total Project Value</label>
                <Input
                  type="number"
              ...
              <div className="mt-6 space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <div className="px-3 py-1 rounded-full text-sm font-medium" 
                    style={{
                      backgroundColor: selectedProject.status === 'completed' ? 'rgb(187 247 208)' : 
                                     selectedProject.status === 'in_progress' ? 'rgb(254 240 138)' :
                                     selectedProject.status === 'on_hold' ? 'rgb(254 202 202)' :
                                     'rgb(226 232 240)',
                      color: selectedProject.status === 'completed' ? 'rgb(22 101 52)' :
                             selectedProject.status === 'in_progress' ? 'rgb(133 77 14)' :
                             selectedProject.status === 'on_hold' ? 'rgb(153 27 27)' :
                             'rgb(51 65 85)'
                    }}>
                  ...
                  <div className="space-y-4">
                    <div className="p-4 border rounded-lg space-y-3">
                      <div>
                        ...
                        <p className="font-medium">{selectedProject.clientId.name}</p>
                        <p className="text-sm text-muted-foreground">{selectedProject.clientId.email}</p>
                        <p className="text-sm text-muted-foreground">{selectedProject.clientId.phone}</p>
                      </div>
                      <div>
                        <span className="text-sm text-muted-foreground">Project Items</span>
                        <p className="font-medium">{selectedProject.items.length} items</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <TableBody>
                  {milestones.map((milestone, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell>
                        {totalProjectValue
                          ? formatCurrency((parseFloat(totalProjectValue) * milestone.percentage) / 100)
                          : '-'}
                      </TableCell>
                      <TableCell>
                        <Button
                  ...
                  <TableRow className="font-medium">
                    ...
                    <TableCell>
                      {totalProjectValue ? formatCurrency(parseFloat(totalProjectValue)) : '-'}
                    </TableCell>
                    <TableCell />
                  </TableRow>
                </TableBody>
