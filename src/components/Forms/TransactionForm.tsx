...
import { CreateManualTransactionData, UpdateTransactionData, Transaction } from "@/api/types"
...
interface TransactionFormProps {
  ...
  onSubmit: (data: CreateManualTransactionData | UpdateTransactionData) => Promise<void>;
  loading: boolean;
  onCancel: () => void;
}
...
export default function TransactionForm({
  ...
  useEffect(() => {
    if (mode === "update" && initialData) {
      ...
      // Set the selected project from initial data
      if (initialData.projectId) {
        setSelectedProject({
          _id: initialData.projectId._id,
          title: initialData.projectId.title,
          location: "",
          status: "",
          startDate: "",
          estimatedEndDate: "",
  ...
  useEffect(() => {
    if (preSelectedProjectId && mode === "create") {
      // Fetch project details if we have a pre-selected project ID
      const fetchProject = async () => {
        try {
          setProjectLoading(true)
          // You might need to create a getProjectById API call
          // For now, we'll search for projects with the ID
          // const response = await projectApi.searchProjects(preSelectedProjectId)
          const response = await projectApi.getProjectById(preSelectedProjectId)
          const projectsData = response.data.data 
          console.log("Fetched project:", projectsData) // Debug log
          if (projectsData) {
            setSelectedProject({
              ...
              estimatedEndDate: projectsData.estimatedEndDate,
              client: projectsData.client || projectsData.clientId, // Handle both cases
            })
          }
        } catch (error) {
  ...
  const searchProjectsAPI = useCallback(async (query: string) => {
    ...
    try {
      ...
      console.log("Project search response:", response.data) // Debug log
      ...
      console.log("Parsed projects:", projectsData) // Debug log
  ...
  // Debounced search function - delays API calls by 500ms
  ...
  const isProjectSelectionDisabled = mode === "update" || !!preSelectedProjectId
              ...
              <div className="p-3 bg-muted rounded-lg">
                ...
                {selectedProject && (
                  <div className="text-sm text-muted-foreground mt-1">
                    Project cannot be changed in {mode} mode
                  </div>
                )}
              </div>
            ) : (
